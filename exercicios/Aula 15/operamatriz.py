#Slide 11 (quase igual ao Slide 19) da Aula 15
def soma(mat1,mat2):
    matsoma=[[mat1[i][j]+mat2[i][j] for j in range(len(mat1[i]))] for i in range(len(mat1))]
    return matsoma

def sub(mat1,mat2):
    matsub=[[mat1[i][j]-mat2[i][j] for j in range(len(mat1[i]))] for i in range(len(mat1))]
    return matsub

def trans(mat):
    mattrans=[[mat[j][i] for j in range(len(mat))] for i in range(len(mat[0]))]
    return mattrans
...
'''matmult=[[(elemento+=mat1[i][k]*mat2[k][j] for k in range(len(mat1[i]))) for k in range(len(mat1[i]))] for i in range(len(mat1))]'''

print('Digite um número para escolher a operação correspondente:')
print('1 - Soma de duas matrizes mxn')
print('2 - Subtração de duas matrizes mxn')
print('3 - Transposição de uma matriz mxn')
print('4 - Multiplicação de duas matrizes mxp e pxn')
escolha=int(input())
if escolha==1 or escolha==2:
    ...
    if escolha==1:
        ...
        print('A subtração das matriz 1 pela 2 é: ', matsub)       
elif escolha==3:
    m=int(input('Digite o número de linhas da matriz: '))
    n=int(input('Digite o número de colunas da matriz: '))
    mat=[]
    print('Digite os elementos, 1 a 1, de cada coluna em cada linha, finalizando com Enter.')
    for i in range(m):
        linha=[]
        for j in range(n):
            elemento=float(input())
            linha.append(elemento)
        mat.append(linha)
    print('A matriz inserida é: ', mat)
    mattrans=trans(mat)
    print('A matriz transposta da inserida é: ', mattrans)
elif escolha==4:
